...
from mi_api.Vendedor.domain.comercio_pedido_domain import OrderStatus, OrderSummary, ProductSummary
from mi_api.shared_store import MERCHANTS, ORDERS
...
class MerchantOrderRepository:
    ...
    def get_orders(self, merchant_id: str, status_filter: Optional[OrderStatus] = None) -> List[OrderSummary]:
    ...
    def _hydrate(self, data: dict) -> OrderSummary:
        return OrderSummary(
            ...
            total=data["total"],
            products=[ProductSummary(p["name"], p["quantity"]) for p in data["products"]],
